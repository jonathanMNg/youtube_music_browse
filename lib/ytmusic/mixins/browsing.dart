...
mixin BrowsingMixin on YTMusicServices {
  Future<Map<String, dynamic>> browse({Map<String, dynamic>? body, int limit = 2, String additionalParams = ''}) async {
    if (additionalParams != '') {
    ...
    if (contents != null) {
      Map<String, dynamic>? tabRenderer = nav(contents,
  ...
  Future addYoutubeHistory(String videoId) async {
    ...
    final Map<String, dynamic> response = await sendRequest('player', body);
    String url =
  ...
  Future<Map<String, dynamic>?> getSongDetails(String videoId) async {
    ...
    final Map<String, dynamic> response = await sendRequest('player', body);
    Map<String, dynamic>? videoDetails = response['videoDetails'];
  ...
  Future<List> getNextSongList({
    ...
    try {
      ...
      final Map<String, dynamic> response = await sendRequest('next', body);
...
List handleChips(chips) {
  List resultChips = [];
  for (Map<String, dynamic> chip in chips) {
...
List<Map<String, dynamic>> handleOuterContents(List contents,
  ...
  for (Map<String, dynamic> content in contents) {
    Map<String, dynamic> result = {};
    result['contents'] = [];
    Map<String, dynamic>? musicPlaylistShelfRenderer =
        nav(content, ['musicPlaylistShelfRenderer']);
    Map<String, dynamic>? musicShelfRenderer = nav(content, ['musicShelfRenderer']);
    Map<String, dynamic>? musicCarouselShelfRenderer =
        nav(content, ['musicCarouselShelfRenderer']);
    Map<String, dynamic>? gridRenderer = nav(content, ['gridRenderer']);
...
handleMusicCarouselShelfRenderer(Map<String, dynamic> item) {
...
Map<String, dynamic> handleHeader(Map<String, dynamic> header) {
